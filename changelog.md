## 5.0.0 - 2026-02-09
* Breaking changes:
* All inner enum/type classes extracted to standalone top-level classes (e.g. `ExtractRequestConfig.ChunkingMethod` → `ExtractRequestConfigChunkingMethod`, `FhirBundle.EntryItem` → `FhirBundleEntryItem`, `WorkflowStep.Type` → `WorkflowStepType`, etc.)
...
* Add `codes` field to `UploadRequest` for direct JSON code array upload (alternative to base64-encoded file)
* Add `async`, `replace`, `revision`, `defnCol` fields to `UploadRequest`
* Improvements:
* Improved retry behavior with `Retry-After` and `X-RateLimit-Reset` header support
* Improved SSE stream parsing
* `RequestOptions` now supports custom query parameters
