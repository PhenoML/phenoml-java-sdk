## 11.0.0 - 2026-04-03
* The SDK's client builder API has been overhauled to support explicit authentication strategies. Use `PhenomlClient.withToken(token)` to authenticate with a pre-generated access token, or `PhenomlClient.withCredentials(clientId, clientSecret)` to authenticate via OAuth client credentials with automatic token refresh. The previous `builder().clientId(...).clientSecret(...)` pattern has been removed — callers must migrate to one of the new factory methods. Additionally, SDK-level request/response logging can now be configured via a `logging(LogConfig)` builder option, and per-request query parameters can be injected through `RequestOptions.Builder.addQueryParameter()`.
* The SDK now intelligently respects API rate-limit response headers (`Retry-After` and `X-RateLimit-Reset`) when determining retry delays, falling back to jittered exponential backoff when headers are absent. SSE streaming now supports event-level discriminated unions via new `Stream.fromSseWithEventDiscrimination()` factory methods, enabling correct deserialization of polymorphic SSE payloads.
* The async agent client now supports a new `list(RequestOptions)` convenience overload for retrieving agents without specifying request parameters. All agent endpoints now forward any custom query parameters provided via `RequestOptions` to the server. Additionally, stream chat requests now use an unlimited call timeout, preventing premature disconnection on long-running streaming responses.
* The `RawAgentClient` now supports a new `list(RequestOptions)` convenience overload, allowing callers to pass request options without constructing an `AgentListRequest`. All agent endpoints now forward query parameters supplied via `RequestOptions` to the underlying HTTP request, and streaming chat connections no longer have a default call timeout applied.
* The `AsyncRawPromptsClient` now supports passing custom query parameters through `RequestOptions` on all agent prompt methods. A new `update(String id, RequestOptions)` convenience overload is also available for updating prompts without specifying a full request body.
* The `AgentChatStreamEvent.Type` inner class has been removed and replaced with a new standalone `AgentChatStreamEventType` class. Callers referencing `AgentChatStreamEvent.Type`, `AgentChatStreamEvent.Type.Value`, or `AgentChatStreamEvent.Type.Visitor` must update their code to use `AgentChatStreamEventType` directly.
* Additionally, header and query fields on request types (`AgentChatRequest`, `AgentStreamChatRequest`, `AgentGetChatMessagesRequest`, `AgentListRequest`) are now correctly excluded from JSON body serialization. A new `additionalProperty` / `additionalProperties` fluent builder API is available on all affected request types, and a new `RawPromptsClient.update(String id, RequestOptions)` convenience overload has been added.
* The nested `Provider`, `Role`, and `Op` types previously defined inside `AgentCreateRequest`, `AgentTemplate`, `ChatMessageTemplate`, and `JsonPatchOperation` have been extracted into dedicated top-level classes (`AgentCreateRequestProvider`, `AgentTemplateProvider`, `ChatMessageTemplateRole`, and `JsonPatchOperationOp`). Callers referencing the old nested types must update their imports and type references to the new top-level class names. A new `getToken(RequestOptions)` overload is also available on `AsyncRawAuthClient`.
* The `RawAuthClient` now exposes a new `getToken(RequestOptions)` overload, allowing callers to retrieve an OAuth token with custom request options without providing explicit credentials. Additionally, `OAuthError`'s inner `Error` type has been extracted into a standalone `OAuthErrorError` class — callers referencing `OAuthError.Error` will need to update to `OAuthErrorError`.
* Several `AsyncRawConstrueClient` and `RawConstrueClient` methods now have convenience overloads that accept a `RequestOptions` argument directly, without requiring a full request builder object. These new overloads are available for `getCodeSystemDetail`, `deleteCustomCodeSystem`, `exportCustomCodeSystem`, `listCodesInACodeSystem`, and `getASpecificCode`. All endpoints also now forward any custom query parameters supplied via `RequestOptions` to the outgoing HTTP request.
* Several new convenience overloads are now available on the Construe client — `getCodeSystemDetail`, `deleteCustomCodeSystem`, `exportCustomCodeSystem`, `listCodesInACodeSystem`, and `getASpecificCode` can each be called with only a `RequestOptions` argument, without needing to construct a full request object. Additionally, query parameters supplied via `RequestOptions` are now forwarded to the server on every endpoint, and request builder classes expose new `additionalProperty` / `additionalProperties` helpers for custom metadata.
* Several nested enum types within `ExtractRequestConfig` and `UploadRequest` have been promoted to standalone top-level classes. Callers referencing `ExtractRequestConfig.ChunkingMethod`, `ExtractRequestConfig.ValidationMethod`, `ExtractRequestConfig.ConsistencyEffort`, or `UploadRequest.Format` must update their imports to use `ExtractRequestConfigChunkingMethod`, `ExtractRequestConfigValidationMethod`, `ExtractRequestConfigConsistencyEffort`, and `UploadRequestFormat` respectively. All other functionality remains the same.
* The `GetCodeSystemDetailResponse.Status` inner class has been renamed to a top-level `GetCodeSystemDetailResponseStatus` type. Callers referencing `GetCodeSystemDetailResponse.Status` must update their code to use `GetCodeSystemDetailResponseStatus` instead. Additionally, response builder types now expose `additionalProperty` and `additionalProperties` helper methods for attaching arbitrary metadata, and list-setter methods now safely handle `null` inputs.
* The FHIR client now supports new convenience overloads for `create`, `upsert`, `delete`, `patch`, and `executeBundle` that accept a resource body directly (e.g. `FhirResource` or `FhirBundle`) without requiring a request wrapper object. All FHIR operations also now correctly forward any query parameters specified in `RequestOptions` to the FHIR server.
* The FHIR client (`RawFhirClient`) now supports convenience overloads for `search`, `create`, `upsert`, `delete`, `patch`, and `executeBundle` that accept a body argument directly, with or without `RequestOptions`, removing the need to manually construct request wrapper objects. Query parameters specified in `RequestOptions` are now forwarded to all FHIR HTTP requests. FHIR request builder classes (`FhirCreateRequest`, `FhirDeleteRequest`, `FhirExecuteBundleRequest`) also expose new `additionalProperty` and `additionalProperties` builder methods.
* The `FhirBundle.EntryItem` inner class (including its nested `Request`, `Response`, and `Method` types) has been removed and replaced by the new top-level `FhirBundleEntryItem` class. Similarly, `FhirPatchRequestBodyItem.Op` has been replaced by the top-level `FhirPatchRequestBodyItemOp` class. Callers referencing `FhirBundle.EntryItem`, `FhirBundle.EntryItem.Request`, `FhirBundle.EntryItem.Response`, `FhirBundle.EntryItem.Request.Method`, or `FhirPatchRequestBodyItem.Op` must update their imports and type references to the new top-level classes.
* Several nested inner classes have been promoted to top-level types to improve clarity and reduce ambiguity. The following renames are breaking:
...
* Update all import statements and type references accordingly.
* The `CreateMultiResponse` and `SearchResponse` types have been refactored so that their nested inner types are now standalone top-level classes. Specifically, `CreateMultiResponse.Bundle` is now `CreateMultiResponseBundle`, `CreateMultiResponse.ResourcesItem` is now `CreateMultiResponseResourcesItem`, and `SearchResponse.ResourceType` is now `SearchResponseResourceType`. Callers that reference these types must update their imports and type annotations accordingly.
* The inner classes `CreateSummaryRequest.Mode`, `CreateSummaryRequest.FhirResources`, and `FhirBundle.EntryItem` have been removed and replaced with top-level types `CreateSummaryRequestMode`, `CreateSummaryRequestFhirResources`, and `FhirBundleEntryItem` respectively. Callers that reference these inner types must update their imports and type references to use the new top-level classes. Additionally, the `X-Phenoml-On-Behalf-Of` and `X-Phenoml-Fhir-Provider` fields on `CohortRequest` and `Lang2FhirAndCreateMultiRequest` are no longer serialized as JSON body properties and are now correctly treated as HTTP headers only.
* Several nested static inner types have been promoted to top-level classes to improve usability and consistency across the SDK. `Lang2FhirAndCreateRequest.Resource` is now `Lang2FhirAndCreateRequestResource`, and `Lang2FhirAndCreateMultiResponse.ResponseBundle` / `ResourceInfoItem` are now `Lang2FhirAndCreateMultiResponseResponseBundle` / `Lang2FhirAndCreateMultiResponseResourceInfoItem` respectively. Callers must update their imports and type references to the new top-level class names. Additionally, the `phenomlOnBehalfOf` and `phenomlFhirProvider` fields on request builders are now correctly excluded from JSON body serialization (they are transmitted as HTTP headers).
* The `getData()` method on `McpServerResponse` now returns `Optional<McpServerResponseData>` (previously `Optional<McpServerResponse.Data>`), and `getData()` on `McpServerToolResponse` now returns `Optional<McpServerToolResponseData>` (previously `Optional<McpServerToolResponse.Data>`). Callers referencing the old inner `Data` types must update their imports and type references to use the new top-level classes `McpServerResponseData` and `McpServerToolResponseData`.
* The `CreateWorkflowRequest.FhirProviderId` and `UpdateWorkflowRequest.FhirProviderId` inner classes have been removed and replaced with new top-level types `CreateWorkflowRequestFhirProviderId` and `UpdateWorkflowRequestFhirProviderId`. Callers using `.fhirProviderId(CreateWorkflowRequest.FhirProviderId.of(...))` or `.fhirProviderId(UpdateWorkflowRequest.FhirProviderId.of(...))` must migrate to the equivalent `.of(...)` factory methods on the new top-level types. Additionally, new `list(RequestOptions)` and `get(String, RequestOptions)` convenience overloads are now available on the workflows client, and all workflow request builders now expose `additionalProperty` and `additionalProperties` helper methods.
* Several nested types within the workflows API have been promoted to standalone top-level classes. `ExecuteWorkflowResponse.Results` is now `ExecuteWorkflowResponseResults`, `WorkflowResponse.Graph` is now `WorkflowResponseGraph`, `WorkflowStep.Type` is now `WorkflowStepType`, and `WorkflowStepSummary.Type` is now `WorkflowStepSummaryType`. Callers that reference the old inner classes must update their imports and type references to use the new top-level names.
* The SDK now includes a configurable logging framework. Use `LogConfig` and `ILogger` to attach a custom logger or enable built-in console logging at a chosen level (`DEBUG`, `INFO`, `WARN`, `ERROR`). By default, logging remains silent. Additionally, a new `SseEvent<T>` class is available for working with Server-Sent Event envelopes, and HTTP error handling is more robust with safer response body reading and graceful error-body parsing.
* The SDK now supports agent chat streaming via a new `SseEventParser` utility and the `AgentChatStreamEventType` enum (`MESSAGE_START`, `CONTENT_DELTA`, `MESSAGE_END`, `ERROR`, `TOOL_USE`, `TOOL_RESULT`). New `AgentCreateRequestProvider` and `AgentTemplateProvider` union types allow provider fields to accept either a single string or a list of strings, and a new `ChatMessageTemplateRole` enum is available for chat session templates. Several client methods now accept an optional `RequestOptions` parameter, and builder classes across request and response types expose new `additionalProperty` and `additionalProperties` methods for attaching arbitrary metadata.
* The SDK now supports `RequestOptions` overloads on `AuthClient`, `AsyncAuthClient`, `ConstrueClient`, and `AsyncConstrueClient`, giving callers per-request control over timeouts and other options. Two new enum types have been added: `JsonPatchOperationOp` (covering add, remove, replace, move, copy, and test operations) and `OAuthErrorError` (covering OAuth 2.0 error codes). All builder classes across the SDK now expose `additionalProperty` and `additionalProperties` convenience methods for setting arbitrary extra fields.
* The SDK now includes several new enum types for configuring extraction requests and code system operations: `ExtractRequestConfigChunkingMethod`, `ExtractRequestConfigConsistencyEffort`, `ExtractRequestConfigValidationMethod`, `GetCodeSystemDetailResponseStatus`, and `UploadRequestFormat`. Builder classes across response and request types now support `additionalProperty` and `additionalProperties` methods for attaching arbitrary metadata. The `AsyncFhirClient` gains new `RequestOptions` overloads for all FHIR operations as well as standalone `create`, `upsert`, and `executeBundle` convenience methods.
* The `FhirClient` now exposes `RequestOptions` overloads for all FHIR operations (`search`, `create`, `upsert`, `delete`, `patch`, and `executeBundle`), giving callers per-request control over timeouts and other options. Several new FHIR model types are also available: `FhirBundleEntryItem`, `FhirBundleEntryItemRequest`, `FhirBundleEntryItemResponse`, `FhirBundleEntryItemRequestMethod`, `FhirPatchRequestBodyItemOp`, and `FhirResourceMeta` to support richer bundle and patch workflows.
* Three new union types are now available in the FHIR provider module: `FhirProviderListResponseFhirProvidersItem` (representing either a `FhirProviderTemplate` or `FhirProviderSandboxInfo`), `FhirProviderResponseData` (same variants), and `FhirQueryResponseData` (representing either a `Map<String, Object>` or a `String`). All FHIR provider and authentication request/response builders now expose `additionalProperty(String, Object)` and `additionalProperties(Map<String, Object>)` helper methods for attaching arbitrary metadata.
* The SDK now exposes new types for multi-document FHIR extraction: `CreateMultiResponseBundle`, `CreateMultiResponseBundleEntryItem`, `CreateMultiResponseBundleEntryItemRequest`, and `CreateMultiResponseResourcesItem` provide structured access to bundle-level results. A new `CreateRequestResource` type enumerates supported FHIR resource kinds (e.g. `AUTO`, `ENCOUNTER`, `PROCEDURE`, `PATIENT`, and more). Additionally, all lang2fhir request and response builder classes now support `additionalProperty` and `additionalProperties` methods for passing arbitrary metadata.
* The SDK now includes several new types to support expanded FHIR and summary capabilities. A new `SearchResponseResourceType` enum covers 30 FHIR resource types for lang2fhir search responses, `CreateSummaryRequestMode` supports `flatten`, `narrative`, and `ips` summary generation modes, and `CreateSummaryRequestFhirResources` allows passing either a `FhirResource` or a `FhirBundle` to summary requests. Additionally, `FhirBundleEntryItem` and `Lang2FhirAndCreateMultiResponseResourceInfoItem` are now available, and all major response and request builders have gained fluent `additionalProperty` / `additionalProperties` helper methods.
* The SDK introduces several new types and capabilities:
* **MCP Server support**: New `McpServerResponseData` and `McpServerToolResponseData` types are available for working with MCP server and tool metadata.
* **FHIR multi-response bundle**: A new `Lang2FhirAndCreateMultiResponseResponseBundle` type and `Lang2FhirAndCreateRequestResource` enum (with 17 FHIR resource type constants including `AUTO`, `PATIENT`, `ENCOUNTER`, and more) support the lang2fhir-and-create multi-response workflow.
* **Per-call request options**: `WorkflowsClient.list(RequestOptions)`, `WorkflowsClient.get(String, RequestOptions)`, and their async counterparts now accept a `RequestOptions` argument for fine-grained control over individual calls.
* Several new types have been added to the workflows resource. `CreateWorkflowRequestFhirProviderId` and `UpdateWorkflowRequestFhirProviderId` are union types that accept either a single `String` or a `List<String>` as a FHIR provider ID. `ExecuteWorkflowResponseResults` exposes per-step execution results, `WorkflowResponseGraph` provides a simplified view of workflow steps, and `WorkflowStepType` / `WorkflowStepSummaryType` enums classify step operations (`SEARCH`, `CREATE`, `WORKFLOW`, `DECISION_NODE`). All existing workflow builder types now also expose `additionalProperty` and `additionalProperties` convenience methods for attaching arbitrary properties.
