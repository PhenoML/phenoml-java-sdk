## [17.13.0] - 2026-08-26
### Added
- **`PatientReference`** — new type with required `system` (identifier namespace) and `value` (identifier value) fields, constructed via a staged builder.
- **`CreateMultiRequest.getPatientReference()` / `patientReference()`** — new optional builder field for attaching a patient identifier to text-based multi-extraction requests.
- **`DocumentMultiRequest.getPatientReference()` / `patientReference()`** — new optional builder field for attaching a patient identifier to document-based multi-extraction requests.

### Changed
- **`ResourceReview`** — now honored by the `/lang2fhir/create/multi` endpoint in addition to `/lang2fhir/document/multi`; the `targets` array is also now constrained to between 1 and 25 items.
...
- **`ResourceReview` / `ResourceReviewTarget` / `ResourceReviewResult` / `ResourceReviewFlagged` / `ResourceReviewFinding`** — new types for the opt-in faithfulness audit that checks whether extracted FHIR resource fields are supported by the source document.
...
- **`CreateMultiRequest.getResourceReview()` / `resourceReview()`** — new optional `ResourceReview` builder field on the single-document multi request.
- **`CreateMultiResponseResourcesItem.getGroup()`** and **`PageClassification.getClassificationId()`** — new optional `String` fields carrying the split-classification id assigned to each extracted resource and page, respectively.
...
* **`ConstrueClient.extractCodes(...)` / `listCodes(...)` / `getCode(...)` / `searchSemantic(...)` / `searchText(...)`** — these five methods were removed from `ConstrueClient` and moved to a new `codes()` sub-client; rewrite as `client.construe().codes().extract(...)` / `list(...)` / `lookup(...)` / `searchSemantic(...)` / `searchText(...)`. `submitFeedback(...)` remains on `ConstrueClient`.
* **`com.phenoml.api.resources.construe.requests.UploadRequest` / `ExtractRequest` / `SearchSemanticRequest` / `SearchTextRequest`** — moved (unrenamed) to `com.phenoml.api.resources.construe.codesystems.requests.UploadRequest` and `com.phenoml.api.resources.construe.codes.requests.{ExtractRequest, SearchSemanticRequest, SearchTextRequest}`; update imports.
* **`com.phenoml.api.resources.construe.requests.GetCodeSystemRequest` / `DeleteCodeSystemRequest` / `ExportCodeSystemRequest`** — renamed and moved to `com.phenoml.api.resources.construe.codesystems.requests.{FindRequest, DeleteRequest, ExportRequest}`; update imports and type references.
* **`com.phenoml.api.resources.construe.requests.ListCodesRequest` / `GetCodeRequest`** — renamed and moved to `com.phenoml.api.resources.construe.codes.requests.{CodesListRequest, LookupRequest}`; update imports and type references.
* **`com.phenoml.api.resources.construe.types.UploadCodeSystemResponse`** — renamed and moved to `com.phenoml.api.resources.construe.codesystems.types.UploadResponse` (now the return type of `codeSystems().upload(...)`); `com.phenoml.api.resources.construe.types.UploadRequestFormat` also moved to `com.phenoml.api.resources.construe.codesystems.types.UploadRequestFormat`. Update imports.
* **`AgentClient.chat(...)` / `streamChat(...)` / `getChatMessages(...)`** — removed from `AgentClient` / `AsyncAgentClient` (and the raw clients) and moved to a new `chat()` sub-client with renamed methods; rewrite as `client.agent().chat().send(...)` / `stream(...)` / `listMessages(...)`.
* **`com.phenoml.api.resources.agent.requests.AgentChatRequest` / `AgentStreamChatRequest`** — moved (unrenamed) to `com.phenoml.api.resources.agent.chat.requests`; update imports.
* **`com.phenoml.api.resources.agent.requests.GetChatMessagesRequest`** — renamed and moved to `com.phenoml.api.resources.agent.chat.requests.ListMessagesRequest`; `GetChatMessagesResponse` / `GetChatMessagesRequestRole` / `GetChatMessagesRequestOrder` are likewise renamed and moved to `com.phenoml.api.resources.agent.chat.types.{ListMessagesResponse, ListMessagesRequestRole, ListMessagesRequestOrder}`. Update imports and type references.
* **`FhirProviderClient.addAuthConfig(...)` / `setActiveAuthConfig(...)` / `removeAuthConfig(...)`** — removed from `FhirProviderClient` / `AsyncFhirProviderClient` (and the raw clients) and moved to a new `authConfig()` sub-client with renamed methods; rewrite as `client.fhirProvider().authConfig().add(...)` / `setActive(...)` / `remove(...)`.
* **`com.phenoml.api.resources.fhirprovider.requests.FhirProviderSetActiveAuthConfigRequest` / `FhirProviderRemoveAuthConfigRequest`** — moved (unrenamed) to `com.phenoml.api.resources.fhirprovider.authconfig.requests`; update imports. `FhirProviderAddAuthConfigRequest` remains in `com.phenoml.api.resources.fhirprovider.types`.
* **`com.phenoml.api.resources.fhirprovider.types.RemoveAuthConfigResponse`** — renamed and moved to `com.phenoml.api.resources.fhirprovider.authconfig.types.RemoveResponse` (now the return type of `authConfig().remove(...)`); update imports.
* **`ToolsClient.mcpServer()`** — removed; the `com.phenoml.api.resources.tools.mcpserver.McpServerClient` (and its nested `tools()` accessor) is split into two sub-clients, `client.tools().mcpServers()` (`com.phenoml.api.resources.tools.mcpservers.McpServersClient`, methods `create`/`list`/`get`/`delete`) and `client.tools().mcpTools()` (`com.phenoml.api.resources.tools.mcptools.McpToolsClient`, methods `list`/`get`/`delete`).
* **`com.phenoml.api.resources.tools.mcpserver.requests.McpServerCreateRequest`** — moved (unrenamed) to `com.phenoml.api.resources.tools.mcpservers.requests`; update imports.
* **`com.phenoml.api.resources.summary.templates.types.TemplatesListResponse` / `TemplatesGetResponse` / `TemplatesUpdateResponse` / `TemplatesDeleteResponse`** — renamed to `ListResponse` / `GetResponse` / `UpdateResponse` / `DeleteResponse` in the same package (the `Templates` prefix added in 16.0.0 is dropped); update imports and references to the return types of `TemplatesClient.list()` / `get(...)` / `update(...)` / `delete(...)`.
* **`FhirClient.create(...)` / `upsert(...)` / `patch(...)` / `executeBundle(...)`** — return type changed from `FhirResource` / `FhirBundle` to `Object`, and the `body` parameter of `create(...)` / `upsert(...)` / `executeBundle(...)` changed from `FhirResource` / `FhirBundle` to `Object`; FHIR proxy payloads are now opaque. Cast or deserialize the `Object` result yourself, and pass a parsed-JSON value (e.g. `Map<String, Object>`) as the body. Affects `AsyncFhirClient` and the raw clients too.
* **`FhirClient.search(...)`** — return type changed from `com.phenoml.api.resources.fhir.types.SearchResponse` to `Object`; deserialize the result yourself.
* **`FhirClient.delete(...)`** — return type changed from `Map<String, Object>` to `Object`.
* **`com.phenoml.api.resources.fhir.requests.SearchRequest`** — the `queryParameters` field (`query_parameters`) and its `getQueryParameters()` / builder `queryParameters(...)` methods were removed; pass FHIR search parameters via the client's request-level query-parameter option (the SDK escape hatch) instead.
* **`com.phenoml.api.resources.fhir.requests.CreateRequest.getBody()` / `UpsertRequest.getBody()` / `ExecuteBundleRequest.getBody()`** — body type changed from `FhirResource` / `FhirBundle` to `Object` and is no longer required (`@NotNull` removed); update builders and accessors.
* **`com.phenoml.api.resources.fhir.types.FhirResource` / `FhirResourceMeta` / `FhirBundle` / `FhirBundleEntryItem` / `FhirBundleEntryItemRequest` / `FhirBundleEntryItemRequestMethod` / `FhirBundleEntryItemResponse` / `SearchResponse` / `ErrorResponse`** — removed; delete all imports and replace with your own FHIR model or generic `Object`/`Map<String, Object>` handling.
* **`com.phenoml.api.resources.fhir.errors.BadGatewayError` / `BadRequestError` / `InternalServerError` / `NotFoundError` / `ServiceUnavailableError` / `TooManyRequestsError` / `UnauthorizedError`** — all FHIR proxy exception classes removed; the proxy now surfaces non-2xx responses via the generic `PhenomlClientApiException`. Remove any `catch` blocks for these types.
* **`com.phenoml.api.resources.lang2fhir.errors.FailedDependencyError`** — removed (the 424 mapping is gone); remove any `catch` blocks for it.
* **`com.phenoml.api.resources.tools.errors.FailedDependencyError`** — removed; remove any `catch` blocks for it.
### Added
* **`client.construe().codeSystems()` / `client.construe().codes()`** — new Construe sub-clients (`CodeSystemsClient` / `CodesClient` with `Async`/`Raw` variants) replacing the flat `ConstrueClient` code methods.
* **`client.agent().chat()`** — new `ChatClient` sub-client (with `Async`/`Raw` variants) exposing `send(...)`, `stream(...)`, and `listMessages(...)`.
* **`client.fhirProvider().authConfig()`** — new `AuthConfigClient` sub-client (with `Async`/`Raw` variants) exposing `add(...)`, `setActive(...)`, and `remove(...)`.
* **`client.tools().mcpServers()` / `client.tools().mcpTools()`** — new `McpServersClient` / `McpToolsClient` sub-clients (with `Async`/`Raw` variants) replacing `tools().mcpServer()` and its nested `tools()` accessor.
* **`com.phenoml.api.resources.agent.errors.GatewayTimeoutError`** — new exception thrown on HTTP 504 from agent endpoints.
* **`com.phenoml.api.resources.lang2fhir.errors.NotFoundError` / `ClientClosedRequestError` / `GatewayTimeoutError`** — new exceptions thrown on HTTP 404 / 499 / 504 from lang2fhir endpoints; 404 and 422 mappings were also added to additional lang2fhir methods.
* **`com.phenoml.api.resources.tools.errors.NotFoundError`** — new exception thrown on HTTP 404 from tools endpoints.
### Changed
* **`com.phenoml.api.resources.workflows.errors.GatewayTimeoutError`** — now also thrown on HTTP 504 by additional `RawWorkflowsClient` methods (previously only `execute(...)`); no API-shape change.
