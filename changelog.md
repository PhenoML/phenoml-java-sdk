## 11.0.0 - 2026-04-03
* The client builder API has been redesigned with explicit, typed authentication entry points. The previous `clientId()` and `clientSecret()` setter methods on `PhenomlClientBuilder` and `AsyncPhenomlClientBuilder` have been removed.
**Migration guide:**
* Replace `PhenomlClient.builder().clientId("id").clientSecret("secret").build()` with `PhenomlClient.withCredentials("id", "secret").build()`
* Replace token-based usage with `PhenomlClient.withToken("token").build()`
* Use `PhenomlClient.builder()` (which now returns a `_Builder`) when you need to configure environment, timeout, or retries before selecting an auth strategy via `.token()` or `.credentials()`
* Additionally, the SDK now supports configurable request/response logging via a new `logging(LogConfig)` builder method, custom instance URLs via `instanceUrl(String)`, and per-request query parameters via `RequestOptions.Builder.addQueryParameter()`.
* The SDK now supports smarter HTTP retry behavior: the retry interceptor respects `Retry-After` and `X-RateLimit-Reset` response headers to determine delay durations, with fallback to capped exponential backoff with jitter. Additionally, a new `SSE_EVENT_DISCRIMINATED` stream mode is available via `Stream.fromSseWithEventDiscrimination()`, enabling proper deserialization of SSE payloads that use event-level discriminated unions.
* A new `list(RequestOptions)` overload is now available on the agent client, allowing callers to pass request options when listing agents without providing an explicit `AgentListRequest`. All agent endpoints now support forwarding query parameters via `RequestOptions`, and streaming chat connections no longer time out prematurely thanks to a disabled call timeout.
* The `RawAgentClient` now supports a new `list(RequestOptions)` overload for listing PhenoAgents without constructing an explicit `AgentListRequest`. Additionally, all agent endpoints now forward query parameters supplied via `RequestOptions`, and streaming chat responses no longer time out prematurely due to an improved OkHttp call-timeout configuration.
* The `AsyncRawPromptsClient` now supports passing query parameters through `RequestOptions` for all agent prompt endpoints. A new `update(String id, RequestOptions requestOptions)` convenience overload is also available, making it easier to update an existing prompt without supplying a full request body.
* The `AgentChatStreamEvent.Type` inner class has been removed and replaced with a new top-level `AgentChatStreamEventType` class. Any code referencing `AgentChatStreamEvent.Type` (including its `Value` enum and `Visitor` interface) must be updated to use `AgentChatStreamEventType` instead. Additionally, header-bearing fields on `AgentChatRequest` and `AgentStreamChatRequest` (such as `phenomlOnBehalfOf` and `phenomlFhirProvider`) are no longer serialized into the JSON request body. A new `update(String id, RequestOptions)` convenience overload is now available on the agent prompts client, and all request builders now expose `additionalProperty` / `additionalProperties` methods for passing arbitrary extra fields.
* Several nested inner types have been extracted into dedicated top-level classes. Callers who previously referenced `AgentCreateRequest.Provider`, `AgentTemplate.Provider`, `ChatMessageTemplate.Role`, or `JsonPatchOperation.Op` must update their imports and references to the new top-level equivalents: `AgentCreateRequestProvider`, `AgentTemplateProvider`, `ChatMessageTemplateRole`, and `JsonPatchOperationOp` respectively. All builder and getter signatures have been updated accordingly.
* The `RawAuthClient` now supports a new `getToken(RequestOptions)` overload, allowing callers to pass request options without specifying a `ClientCredentialsRequest`. Query parameters defined in `RequestOptions` are now forwarded on all auth token requests. The `OAuthError.error` field type has changed from the inner `OAuthError.Error` class to the new top-level `OAuthErrorError` type — callers referencing `OAuthError.Error` directly will need to update to `OAuthErrorError`.
* Several `AsyncRawConstrueClient` methods now have additional convenience overloads that accept only a `RequestOptions` parameter, making it easier to invoke `getCodeSystemDetail`, `deleteCustomCodeSystem`, `exportCustomCodeSystem`, `listCodesInACodeSystem`, and `getASpecificCode` without constructing a full request object. Custom query parameters supplied via `RequestOptions` are now forwarded to all API requests.
* Several Construe client methods now support a new convenience overload that accepts only the required path parameters and a `RequestOptions` argument, without needing to construct a full request object. Affected methods include `getCodeSystemDetail`, `deleteCustomCodeSystem`, `exportCustomCodeSystem`, `listCodesInACodeSystem`, and `getASpecificCode`. Query parameters supplied via `RequestOptions` are now correctly forwarded on all requests.
* Several nested enum types in `ExtractRequestConfig` and `UploadRequest` have been promoted to top-level classes. Callers must update references as follows: `ExtractRequestConfig.ChunkingMethod` → `ExtractRequestConfigChunkingMethod`, `ExtractRequestConfig.ValidationMethod` → `ExtractRequestConfigValidationMethod`, `ExtractRequestConfig.ConsistencyEffort` → `ExtractRequestConfigConsistencyEffort`, and `UploadRequest.Format` → `UploadRequestFormat`. All existing enum values and visitor interfaces remain available on the new top-level classes.
* The `GetCodeSystemDetailResponse.Status` inner type has been replaced by the standalone `GetCodeSystemDetailResponseStatus` class. Callers referencing `GetCodeSystemDetailResponse.Status` must update their imports and type references to use `GetCodeSystemDetailResponseStatus` instead.
* The FHIR client now exposes additional convenience overloads for `search`, `create`, `upsert`, `delete`, `patch`, and `executeBundle` that accept resource body objects directly (e.g. `FhirResource`, `FhirBundle`, `List<FhirPatchRequestBodyItem>`) instead of requiring a request wrapper. All FHIR operations also now correctly forward query parameters supplied via `RequestOptions`.
* The `RawFhirClient` now provides additional convenience overloads for all FHIR operations (`search`, `create`, `upsert`, `delete`, `patch`, and `executeBundle`), allowing callers to pass body types directly without constructing a wrapper request object. All methods now also support an optional `RequestOptions` parameter for per-call configuration, including custom query parameters that are forwarded to the FHIR server. Additionally, `FhirCreateRequest`, `FhirDeleteRequest`, and `FhirExecuteBundleRequest` builders expose new `additionalProperty` and `additionalProperties` methods for attaching arbitrary metadata.
* The `FhirBundle.EntryItem` inner class has been replaced by a new top-level `FhirBundleEntryItem` class, and `FhirBundle.getEntry()` now returns `List<FhirBundleEntryItem>`. Similarly, `FhirPatchRequestBodyItem.Op` has been replaced by the top-level `FhirPatchRequestBodyItemOp` type. Callers referencing these removed inner types or the old `getOp()` / `op()` builder signatures must update their imports and type references to use the new top-level classes.
* Several nested inner types have been extracted into dedicated top-level classes. `FhirResource.Meta` is now `FhirResourceMeta`, `FhirProviderListResponse.FhirProvidersItem` is now `FhirProviderListResponseFhirProvidersItem`, `FhirProviderResponse.Data` is now `FhirProviderResponseData`, `FhirQueryResponse.Data` is now `FhirQueryResponseData`, and `CreateRequest.Resource` is now `CreateRequestResource`. Callers that reference the old nested types must update their imports and type references to use the new top-level equivalents.
* The `CreateMultiResponse` and `SearchResponse` types have been refactored to move nested inner classes to top-level types. `CreateMultiResponse.Bundle` is now `CreateMultiResponseBundle`, `CreateMultiResponse.ResourcesItem` is now `CreateMultiResponseResourcesItem`, and `SearchResponse.ResourceType` is now `SearchResponseResourceType`. Callers must update any references to these types and their corresponding builder method signatures.
* Several request and type classes in the summary and tools APIs have been restructured. The inner classes `CreateSummaryRequest.Mode`, `CreateSummaryRequest.FhirResources`, and `FhirBundle.EntryItem` have been removed and replaced by the new top-level types `CreateSummaryRequestMode`, `CreateSummaryRequestFhirResources`, and `FhirBundleEntryItem` respectively. Callers referencing the old inner classes must update their imports and type references to use the new top-level equivalents. All request builders now also expose `additionalProperty` and `additionalProperties` convenience methods for passing custom fields.
* Several nested static inner classes have been extracted into standalone top-level types. `Lang2FhirAndCreateRequest.Resource` is now `Lang2FhirAndCreateRequestResource`, and `Lang2FhirAndCreateMultiResponse.ResponseBundle` and `Lang2FhirAndCreateMultiResponse.ResourceInfoItem` are now `Lang2FhirAndCreateMultiResponseResponseBundle` and `Lang2FhirAndCreateMultiResponseResourceInfoItem` respectively. Callers referencing these nested types must update their imports and type references to use the new top-level classes.
* The `McpServerResponse.Data` and `McpServerToolResponse.Data` inner classes have been removed and replaced with top-level types `McpServerResponseData` and `McpServerToolResponseData`. Callers that reference `McpServerResponse.Data` or `McpServerToolResponse.Data` must update their imports and type references to use the new top-level classes, which expose identical fields and methods.
* The `fhirProviderId` field in `CreateWorkflowRequest` and `UpdateWorkflowRequest` previously accepted the inner union type `CreateWorkflowRequest.FhirProviderId` / `UpdateWorkflowRequest.FhirProviderId`. These inner classes have been removed and replaced with new top-level types `CreateWorkflowRequestFhirProviderId` and `UpdateWorkflowRequestFhirProviderId`. Callers must update any references to the old inner classes to use the new top-level equivalents. Additionally, new `list(RequestOptions)` and `get(String, RequestOptions)` convenience overloads are now available on the workflows client.
* Several workflow response types have been renamed as part of a refactor that promotes inner classes to top-level types. The following breaking changes require migration:
* `ExecuteWorkflowResponse.Results` → `ExecuteWorkflowResponseResults` (affects `getResults()`, builder `results(...)` methods)
* `WorkflowResponse.Graph` → `WorkflowResponseGraph` (affects `getGraph()`, builder `graph(...)` methods)
* `WorkflowStep.Type` → `WorkflowStepType` (affects `getType()`, builder `type(...)` methods)
* `WorkflowStepSummary.Type` → `WorkflowStepSummaryType` (affects `getType()`, builder `type(...)` methods)
* Update any references to these inner classes to use the new top-level class names.
* The SDK now includes a structured logging framework (`ILogger`, `LogConfig`, `LogLevel`, `Logger`, `ConsoleLogger`, `LoggingInterceptor`) that is silent by default and can be configured to log HTTP requests and responses at various levels, with automatic redaction of sensitive headers. New core utilities have also been added: `SseEvent<T>` for Server-Sent Event support, `Rfc2822DateTimeDeserializer` for RFC 2822 date parsing, and a `DoubleSerializer` that serializes whole-number doubles as integers.
* The SDK now supports agent chat streaming with a new `AgentChatStreamEventType` enum (`MESSAGE_START`, `CONTENT_DELTA`, `MESSAGE_END`, `ERROR`, `TOOL_USE`, `TOOL_RESULT`) and a new `ChatMessageTemplateRole` enum (`USER`, `ASSISTANT`, `MODEL`, `FUNCTION`). New `AgentCreateRequestProvider` and `AgentTemplateProvider` union types allow provider fields to accept either a single string or a list of strings. `AgentClient`, `AsyncAgentClient`, `PromptsClient`, and `AsyncPromptsClient` now expose overloaded `list` and `update` methods that accept a `RequestOptions` parameter for per-request configuration.
* The SDK now supports per-call `RequestOptions` on additional client methods, including `getToken`, `getCodeSystemDetail`, `deleteCustomCodeSystem`, `exportCustomCodeSystem`, `listCodesInACodeSystem`, and `getASpecificCode`. Two new enum types are available: `JsonPatchOperationOp` (for JSON Patch operations) and `OAuthErrorError` (for OAuth 2.0 error codes). All request and response builder types now expose `additionalProperty` and `additionalProperties` convenience methods for setting arbitrary extra fields.
* The SDK now exposes several new types and builder conveniences. Five new enum-style classes are available — `ExtractRequestConfigChunkingMethod`, `ExtractRequestConfigConsistencyEffort`, `ExtractRequestConfigValidationMethod`, `GetCodeSystemDetailResponseStatus`, and `UploadRequestFormat` — enabling strongly-typed access to previously untyped API fields. Builder classes for `DeleteCodeSystemResponse`, `ExtractedCodeResult`, `FeedbackResponse`, `GetCodeResponse`, `SemanticSearchResult`, `TextSearchResult`, and `ExtractRequestSystem` now support `additionalProperty` and `additionalProperties` helpers for attaching arbitrary metadata. `AsyncFhirClient` also gains new overloads for `create`, `upsert`, `patch`, `delete`, `search`, and `executeBundle` that accept an optional `RequestOptions` argument.
* The `FhirClient` now supports `RequestOptions` on all FHIR operations (`search`, `create`, `upsert`, `delete`, `patch`, and `executeBundle`), enabling per-request timeout and header customization. Several new types are available to support structured FHIR bundle and JSON Patch workflows: `FhirBundleEntryItem`, `FhirBundleEntryItemRequest`, `FhirBundleEntryItemResponse`, `FhirBundleEntryItemRequestMethod`, `FhirPatchRequestBodyItemOp`, and `FhirResourceMeta`.
* Three new FHIR provider data types are now available: `FhirProviderListResponseFhirProvidersItem` and `FhirProviderResponseData` (each representing either a `FhirProviderTemplate` or `FhirProviderSandboxInfo`), and `FhirQueryResponseData` (representing either a FHIR resource map or a raw string). All FHIR provider builder classes now support `additionalProperty(String, Object)` and `additionalProperties(Map<String, Object>)` methods for passing through custom fields.
* The SDK now includes new response types for multi-document FHIR conversion: `CreateMultiResponseBundle`, `CreateMultiResponseBundleEntryItem`, `CreateMultiResponseBundleEntryItemRequest`, and `CreateMultiResponseResourcesItem`. A new `CreateRequestResource` enum is also available, providing a typed set of 17 supported FHIR resource types (e.g. `AUTO`, `ENCOUNTER`, `PROCEDURE`, `MEDICATIONREQUEST`). Additionally, all lang2fhir request and response builders now expose `additionalProperty` and `additionalProperties` methods for passing custom key-value data.
* The SDK now includes several new FHIR-related types and capabilities. A `SearchResponseResourceType` enum covers the full set of supported FHIR resource types for lang2fhir search responses. The summary API gains `CreateSummaryRequestMode` (supporting `flatten`, `narrative`, and `ips` modes), `CreateSummaryRequestFhirResources` (a union type accepting either a `FhirResource` or `FhirBundle`), and `FhirBundleEntryItem` for bundle entry representation. The tools API adds `Lang2FhirAndCreateMultiResponseResourceInfoItem` to surface per-resource metadata (temporary ID, resource type, and source text description) from multi-resource creation responses. Builder classes across the summary and tools packages also gain `additionalProperty` and `additionalProperties` convenience methods.
* The SDK now includes two new MCP server response types — `McpServerResponseData` and `McpServerToolResponseData` — to support working with MCP server and tool metadata. A new `Lang2FhirAndCreateMultiResponseResponseBundle` type and `Lang2FhirAndCreateRequestResource` value class (covering 17 FHIR resource kinds) are also available for multi-resource FHIR creation workflows. Additionally, `WorkflowsClient` and `AsyncWorkflowsClient` now expose `list(RequestOptions)` and `get(String, RequestOptions)` overloads for per-call request configuration.
* The SDK introduces several new workflow types. `CreateWorkflowRequestFhirProviderId` and `UpdateWorkflowRequestFhirProviderId` are new union types that allow FHIR provider IDs to be specified as either a single `String` or a `List<String>`. Two new response types are also available: `ExecuteWorkflowResponseResults` (providing per-step execution results keyed by step ID) and `WorkflowResponseGraph` (a simplified graph of workflow steps). New `WorkflowStepType` and `WorkflowStepSummaryType` enums enumerate the supported step kinds (`SEARCH`, `CREATE`, `WORKFLOW`, `DECISION_NODE`). All existing workflow builder classes now also expose `additionalProperty` and `additionalProperties` convenience methods for easier programmatic construction.
