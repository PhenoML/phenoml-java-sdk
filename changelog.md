## 11.0.0 - 2026-04-03
* The `PhenomlClient` and `AsyncPhenomlClient` builder API has been redesigned to make authentication configuration more explicit. The `builder()` method now returns a `_Builder` instance; use `PhenomlClient.withToken(token)` to authenticate with a pre-generated access token, or `PhenomlClient.withCredentials(clientId, clientSecret)` for OAuth client credentials flow. The previous `clientId()` and `clientSecret()` methods on the builder have been removed. Additionally, the SDK now supports configurable logging via a new `logging(LogConfig)` option, per-instance URL configuration via `instanceUrl(String)`, and per-request query parameter injection via `RequestOptions.Builder.addQueryParameter()`.
* The SDK now supports smarter HTTP retry behavior: retry delays are determined by inspecting `Retry-After` and `X-RateLimit-Reset` response headers before falling back to exponential backoff (capped at 60 seconds with jitter). Additionally, a new `SSE_EVENT_DISCRIMINATED` stream mode is available via `Stream.fromSseWithEventDiscrimination()`, enabling correct deserialization of SSE streams whose payloads are discriminated unions keyed on the SSE `event` field.
* A new `list(RequestOptions)` convenience overload is now available on the agent client, making it easier to pass request options when listing PhenoAgents without specifying filter criteria. All agent endpoints now support forwarding custom query parameters through `RequestOptions`, and streaming chat connections no longer time out prematurely.
* The `RawAgentClient` now supports a new `list(RequestOptions)` overload, allowing callers to pass request options directly without constructing an `AgentListRequest`. All agent endpoints also now forward any query parameters specified in `RequestOptions` to the server, and streaming chat connections no longer time out prematurely.
* The `AsyncRawPromptsClient` now supports a new `update(id, RequestOptions)` overload for updating prompts without providing an explicit request body. Additionally, all prompts endpoints now forward any custom query parameters supplied via `RequestOptions` to the underlying HTTP requests.
* The `AgentChatStreamEvent.Type` inner class has been removed and replaced by a new top-level `AgentChatStreamEventType` class. Callers that reference `AgentChatStreamEvent.Type` (or its `Value` enum and `Visitor` interface) must update their imports and references to use `AgentChatStreamEventType` instead. Additionally, `AgentChatRequest`, `AgentStreamChatRequest`, `AgentGetChatMessagesRequest`, and `AgentListRequest` builders now expose `additionalProperty` and `additionalProperties` methods, and all agent-prompts endpoints forward `RequestOptions` query parameters onto the request URL.
* Several nested inner types have been promoted to top-level classes with updated names. `AgentCreateRequest.Provider` is now `AgentCreateRequestProvider`, `AgentTemplate.Provider` is now `AgentTemplateProvider`, `ChatMessageTemplate.Role` is now `ChatMessageTemplateRole`, and `JsonPatchOperation.Op` is now `JsonPatchOperationOp`. Callers must update their imports and type references to use the new top-level classes. Additionally, a new `getToken(RequestOptions)` overload is available on the async auth client, and builder methods for `additionalProperty` / `additionalProperties` have been added to several request types.
* The `OAuthError` type's inner `Error` class has been replaced with a new top-level `OAuthErrorError` type. Callers referencing `OAuthError.Error` (including its constants, `getEnumValue()`, `visit()`, and `valueOf()` methods) must be updated to use `OAuthErrorError` instead. Additionally, a new `getToken(RequestOptions)` overload is now available on `RawAuthClient` for convenience, and auth endpoints now forward query parameters supplied via `RequestOptions`.
* Several new convenience overloads are now available on the async Construe client: `getCodeSystemDetail`, `deleteCustomCodeSystem`, `exportCustomCodeSystem`, `listCodesInACodeSystem`, and `getASpecificCode` each accept a `RequestOptions` parameter directly, without requiring a full request object. Additionally, custom query parameters supplied via `RequestOptions` are now forwarded on all HTTP requests.
* Several Construe API methods now have additional convenience overloads that accept only the required path parameters and a `RequestOptions` argument, removing the need to construct an empty request object. These new overloads are available for `getCodeSystemDetail`, `deleteCustomCodeSystem`, `exportCustomCodeSystem`, `listCodesInACodeSystem`, and `getASpecificCode`. Additionally, query parameters supplied via `RequestOptions` are now correctly forwarded to the server for all endpoints.
* Several nested enum types within `ExtractRequestConfig` and `UploadRequest` have been promoted to standalone top-level classes. `ExtractRequestConfig.ChunkingMethod`, `ExtractRequestConfig.ValidationMethod`, and `ExtractRequestConfig.ConsistencyEffort` are now `ExtractRequestConfigChunkingMethod`, `ExtractRequestConfigValidationMethod`, and `ExtractRequestConfigConsistencyEffort` respectively. `UploadRequest.Format` is now `UploadRequestFormat`. Callers must update all references to these types to use the new top-level class names.
* The `GetCodeSystemDetailResponse.Status` inner class has been renamed to `GetCodeSystemDetailResponseStatus`. Callers that reference `GetCodeSystemDetailResponse.Status` (or its nested `Value` enum and `Visitor` interface) must update their imports and type references to use `GetCodeSystemDetailResponseStatus` instead. Additionally, all construe response builder types (`GetCodeSystemDetailResponse`, `ListCodeSystemsResponse`, `ListCodesResponse`, `SemanticSearchResponse`, and `TextSearchResponse`) now expose `additionalProperty` and `additionalProperties` helper methods on their builders for attaching arbitrary metadata.
* The FHIR client methods (`search`, `create`, `upsert`, `delete`, `patch`, `executeBundle`) now support additional convenience overloads that accept typed body arguments directly, without requiring callers to construct request wrapper objects. All FHIR operations also now forward any query parameters specified in `RequestOptions` to the outgoing HTTP request.
* The `RawFhirClient` now supports convenience overloads for `search`, `create`, `upsert`, `delete`, `patch`, and `executeBundle` that accept body/resource arguments directly, with or without `RequestOptions`, eliminating the need to manually construct request wrapper objects for simple calls. Query parameters specified in `RequestOptions` are now forwarded to the FHIR server for all operations. A null-safety fix also prevents potential errors when reading error response bodies.
* The `FhirBundle.EntryItem`, `FhirBundle.EntryItem.Request`, `FhirBundle.EntryItem.Response`, and `FhirBundle.EntryItem.Request.Method` inner classes have been removed and replaced with a new top-level `FhirBundleEntryItem` type. Similarly, `FhirPatchRequestBodyItem.Op` has been replaced by a new top-level `FhirPatchRequestBodyItemOp` class. Callers that reference these inner types must update to the new top-level equivalents. Additionally, `additionalProperty` and `additionalProperties` builder methods are now available on `FhirPatchRequest`, `FhirSearchRequest`, `FhirUpsertRequest`, `FhirBundle`, and `FhirPatchRequestBodyItem` builders.
* Several nested inner types used in FHIR-related response and request classes have been extracted into top-level classes. `FhirResource.Meta` is now `FhirResourceMeta`, `FhirProviderListResponse.FhirProvidersItem` is now `FhirProviderListResponseFhirProvidersItem`, `FhirProviderResponse.Data` is now `FhirProviderResponseData`, `FhirQueryResponse.Data` is now `FhirQueryResponseData`, and `CreateRequest.Resource` is now `CreateRequestResource`. Callers referencing the old nested types must update their imports and type references to the new top-level equivalents.
* The nested types `CreateMultiResponse.Bundle`, `CreateMultiResponse.ResourcesItem`, and `SearchResponse.ResourceType` have been removed and replaced with new top-level types `CreateMultiResponseBundle`, `CreateMultiResponseResourcesItem`, and `SearchResponseResourceType`. Update any code that references these inner classes to use the new top-level equivalents, and update relevant builder method calls accordingly.
* The `CreateSummaryRequest.Mode` and `CreateSummaryRequest.FhirResources` inner classes have been extracted to top-level types `CreateSummaryRequestMode` and `CreateSummaryRequestFhirResources` respectively. Similarly, `FhirBundle.EntryItem` is now `FhirBundleEntryItem`. Callers referencing these types by their former inner-class names (e.g. `CreateSummaryRequest.Mode`, `CreateSummaryRequest.FhirResources`, `FhirBundle.EntryItem`) must update their imports and references to the new top-level class names.
* The `Lang2FhirAndCreateRequest.Resource`, `Lang2FhirAndCreateMultiResponse.ResponseBundle`, and `Lang2FhirAndCreateMultiResponse.ResourceInfoItem` nested types have been extracted into standalone top-level classes (`Lang2FhirAndCreateRequestResource`, `Lang2FhirAndCreateMultiResponseResponseBundle`, and `Lang2FhirAndCreateMultiResponseResourceInfoItem`). Callers must update their type references and imports accordingly. Additionally, a serialization bug has been fixed where the `X-Phenoml-On-Behalf-Of` and `X-Phenoml-Fhir-Provider` authentication header fields were incorrectly included in the JSON request body; they are now correctly excluded from serialization.
* The `getData()` method on `McpServerResponse` now returns `Optional<McpServerResponseData>` (previously `Optional<McpServerResponse.Data>`), and the same change applies to `McpServerToolResponse`, which now returns `Optional<McpServerToolResponseData>` (previously `Optional<McpServerToolResponse.Data>`). Callers that reference the old nested `Data` inner classes must update their imports and type references to use the new top-level types `McpServerResponseData` and `McpServerToolResponseData`.
* The `fhirProviderId` field type in `CreateWorkflowRequest` and `UpdateWorkflowRequest` has changed from the inner class `CreateWorkflowRequest.FhirProviderId` / `UpdateWorkflowRequest.FhirProviderId` to new top-level types `CreateWorkflowRequestFhirProviderId` and `UpdateWorkflowRequestFhirProviderId` respectively. Callers that reference the old inner types (e.g. `CreateWorkflowRequest.FhirProviderId.of(...)`) must update their imports and usages to the new top-level classes. The builder interfaces for these requests also now accept the new types, so any code passing the old types will fail to compile.
* Several nested inner types within workflow response classes have been renamed and promoted to top-level classes. `ExecuteWorkflowResponse.Results` is now `ExecuteWorkflowResponseResults`, `WorkflowResponse.Graph` is now `WorkflowResponseGraph`, `WorkflowStep.Type` is now `WorkflowStepType`, and `WorkflowStepSummary.Type` is now `WorkflowStepSummaryType`. Callers referencing these inner classes (e.g. `ExecuteWorkflowResponse.Results`, `WorkflowStep.Type.WORKFLOW`) must update their imports and type references to use the new top-level class names.
* The SDK now includes a configurable logging framework. Use the new `LogConfig`, `LogLevel`, and `ILogger` types to enable and customize HTTP request/response logging — sensitive headers (e.g. `Authorization`, `X-Api-Key`) are automatically redacted. Logging is silent by default and must be explicitly enabled. Additionally, integer-valued `double` fields are now serialized without a trailing decimal point (e.g. `24000.0` becomes `24000`), and `RequestOptions` query parameters are now correctly forwarded on all MCP server tool requests.
* The SDK now supports agent chat streaming via a new `AgentChatStreamEventType` enum (`MESSAGE_START`, `CONTENT_DELTA`, `MESSAGE_END`, `ERROR`, `TOOL_USE`, `TOOL_RESULT`) and an internal `SseEventParser` utility for Server-Sent Events. New union types `AgentCreateRequestProvider` and `AgentTemplateProvider` allow the provider field to accept either a `String` or a `List<String>`. A new `ChatMessageTemplateRole` enum is also available. Additionally, `list()` and `update()` methods on `AgentClient`, `AsyncAgentClient`, `PromptsClient`, and `AsyncPromptsClient` now have overloads accepting `RequestOptions`, and all builder types expose `additionalProperty`/`additionalProperties` methods for passing through extra fields.
* Several new capabilities have been added across the SDK. All client methods now support an optional `RequestOptions` parameter, enabling per-request configuration on `AuthClient`, `AsyncAuthClient`, `ConstrueClient`, and `AsyncConstrueClient`. Two new enum types are available: `JsonPatchOperationOp` (for JSON Patch operations) and `OAuthErrorError` (for OAuth error classification). Additionally, all request and response builder types now expose `additionalProperty` and `additionalProperties` helper methods for passing custom fields.
* Five new enum types are now available: `ExtractRequestConfigChunkingMethod`, `ExtractRequestConfigConsistencyEffort`, `ExtractRequestConfigValidationMethod`, `GetCodeSystemDetailResponseStatus`, and `UploadRequestFormat`. Builder classes on `DeleteCodeSystemResponse`, `ExtractedCodeResult`, `FeedbackResponse`, `GetCodeResponse`, `SemanticSearchResult`, `TextSearchResult`, and `ExtractRequestSystem` now expose `additionalProperty` and `additionalProperties` helper methods. `AsyncFhirClient` gains new overloads for `search`, `create`, `upsert`, `delete`, `patch`, and `executeBundle` — including both `RequestOptions`-accepting and convenience variants.
* The `FhirClient` now exposes `RequestOptions`-accepting overloads for all FHIR operations (`search`, `create`, `upsert`, `delete`, `patch`, and `executeBundle`), giving callers per-request control over timeouts and other options. Several new supporting types have also been introduced: `FhirBundleEntryItem`, `FhirBundleEntryItemRequest`, `FhirBundleEntryItemResponse`, `FhirBundleEntryItemRequestMethod`, `FhirPatchRequestBodyItemOp`, and `FhirResourceMeta`.
* Three new union types are now available in the FHIR provider package: `FhirProviderListResponseFhirProvidersItem`, `FhirProviderResponseData`, and `FhirQueryResponseData`, enabling typed access to polymorphic FHIR provider response payloads. All FHIR provider builder classes now expose `additionalProperty` and `additionalProperties` convenience methods for attaching arbitrary metadata to requests.
* The SDK now includes new types for multi-document FHIR bundle responses: `CreateMultiResponseBundle`, `CreateMultiResponseBundleEntryItem`, `CreateMultiResponseBundleEntryItemRequest`, and `CreateMultiResponseResourcesItem`. A new `CreateRequestResource` enum is available with 17 supported FHIR resource type values (e.g. `AUTO`, `MEDICATIONREQUEST`, `ENCOUNTER`, `PROCEDURE`). Additionally, all lang2fhir request builders (`DocumentMultiRequest`, `DocumentRequest`, `ProfileUploadRequest`, `SearchRequest`) and `Lang2FhirUploadProfileResponse` now expose `additionalProperty` and `additionalProperties` builder methods for passing through custom fields.
* The SDK now includes several new types to support expanded FHIR resource handling. A `SearchResponseResourceType` enum covering 30 FHIR resource types is available in the `lang2fhir` package, and the `summary` package gains `CreateSummaryRequestFhirResources` (a union of `FhirResource` and `FhirBundle`), `CreateSummaryRequestMode` (`FLATTEN`, `NARRATIVE`, `IPS`), and `FhirBundleEntryItem`. A new `Lang2FhirAndCreateMultiResponseResourceInfoItem` type exposes `tempId`, `resourceType`, and `description` fields from multi-resource creation responses. Builder classes across multiple response types also now expose `additionalProperty` and `additionalProperties` convenience methods.
* The SDK introduces several new types and capabilities for MCP server and FHIR workflow integrations. New classes `McpServerResponseData` and `McpServerToolResponseData` represent MCP server and tool metadata, while `Lang2FhirAndCreateMultiResponseResponseBundle` supports multi-resource FHIR bundle responses. A new `Lang2FhirAndCreateRequestResource` type provides constants for 17 FHIR resource types (such as `MEDICATIONREQUEST`, `ENCOUNTER`, `PATIENT`, and `AUTO`). Additionally, `WorkflowsClient` and `AsyncWorkflowsClient` now expose `list(RequestOptions)` and `get(String, RequestOptions)` overloads for per-call request configuration.
* Several new workflow types are now available in the SDK:
* `CreateWorkflowRequestFhirProviderId` and `UpdateWorkflowRequestFhirProviderId` are new union types that accept either a single `String` or a `List<String>` of FHIR provider IDs when creating or updating workflows.
* `ExecuteWorkflowResponseResults` exposes per-step execution results keyed by step ID, and `WorkflowResponseGraph` provides a simplified view of workflow steps without full operation details.
* `WorkflowStepType` and `WorkflowStepSummaryType` are new enums with values `SEARCH`, `CREATE`, `WORKFLOW`, and `DECISION_NODE`.
* All workflow builder types now expose `additionalProperty` and `additionalProperties` convenience methods for setting arbitrary extra fields.
