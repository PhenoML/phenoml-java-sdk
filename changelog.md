...
* **`com.phenoml.api.resources.construe.requests.ListCodesRequest` / `GetCodeRequest`** — renamed and moved to `com.phenoml.api.resources.construe.codes.requests.{ListRequest, LookupRequest}`; update imports and type references.
...
* **`com.phenoml.api.resources.agent.types.ListResponse` / `DeleteResponse`** — renamed to `AgentListResponse` / `AgentDeleteResponse` in the same package (the `Agent` prefix dropped in 16.0.0 is re-added); update imports and any references to the return types of `AgentClient.list(...)` / `delete(...)`. Note `agent.requests.ListRequest` is unchanged.
* **`com.phenoml.api.resources.agent.prompts.types.PromptsListResponse` / `PromptsDeleteResponse`** — renamed to `PromptListResponse` / `PromptDeleteResponse` (singular) in the same package; update imports and references to the return types of `PromptsClient.list(...)` / `delete(...)`.
